...
function Camera({ name, config, availableWidth }) {
  ...
  const buttons = useMemo(() => {
    return [
      { name: '事件记录', href: `/events?cameras=${name}` },
      { name: '录像文件', href: `/recording/${name}` },
    ];
  }, [name]);
    ...
    () => (availableWidth < 448 ? [
      ...
      },
      {
        name: config.record.enabled_in_config
          ? `${recordValue === 'ON' ? '关闭' : '开启'}录像`
          : '需在配置文件中启用录像功能后，方可在界面操作。',
        icon: ClipIcon,
      ...
      {
        name: `${snapshotValue === 'ON' ? '关闭' : '开启'}快照`,
        icon: SnapshotIcon,
